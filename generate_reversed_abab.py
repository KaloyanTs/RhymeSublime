...
def extract_rime_ltr(line_text_ltr: str, *, stress_predict_fn=None, stress_dict=None):
    """
    Finds last Cyrillic word, returns (tail_from_stress, stress_idx). LTR input.
    """
...
def generateText_rtl_abab(
    ...
    *,
    ...
    def step_sample(prev_id: int, h0, c0, prev_tok: str):
        ...
        if debug:
            ch = id2tok[nxt] if 0 <= nxt < len(id2tok) else "?"
    ...
    while len(out_rtl) < int(limit):
        if debug:
            mode = (line_no % 4)
            print(f"[Loop] line_no={line_no+1} ABAB_stage={mode}")
        forced = None
        forced_chars = None
        toks = None
        ended = False
        ...
        if stage in (0, 1):
            ...
            if tail_ltr:
                forced_tail_rtl = tail_ltr[::-1]
                forced_chars = list(forced_tail_rtl)
                if debug:
                    print(f"[Loop] forced_tail_ltr='{tail_ltr}' forced_prefix_rtl='{forced_tail_rtl}'")
            if int(K) <= 1:
                toks, ll, h, c, ended = sample_line(
                    h, c, last_id, last_tok,
                    forced_prefix_ids=forced,
                ...
                for k in range(int(K)):
                    ...
                    denom = max(1, len(tk))
                    ll_norm = ll_k / denom
                    rhyme_loss = float(rhyme_dp_penalty(tail_ltr or "", cand_tail_k or ""))
                    rep_pen = repetition_penalty(tk[:-1], w_rep, id2tok)
                    score = float(w_ll) * ll_norm - float(w_rhyme) * rhyme_loss - float(w_rep) * rep_pen
                    if last_word_base and cand_word_k and last_word_base == cand_word_k:
                        score -= 20.0
                    if debug:
                        print(f"[ABAB] K={k+1}/{K} word={repr(cand_word_k)} cand_tail={repr(cand_tail_k)} ll_norm={ll_norm:.4f} rhyme={rhyme_loss:.4f} rep={rep_pen:.4f} score={score:.4f}")
                    if best is None or score > best[0]:
                        best = (score, tk)
                        best_pack = (hk, ck, endk)
                if debug and best is not None:
                    print(f"[ABAB] chosen score={best[0]:.4f}")
        ...
        line_ltr = line_rtl[::-1]
        if stage == 0:
            base_tail_A, _ = extract_rime_ltr(line_ltr, stress_predict_fn=stress_predict_fn, stress_dict=stress_dict)
            base_last_word_A = extract_last_word_ltr(line_ltr)
            if debug:
                print(f"[ABAB] Line {line_no+1} base A tail={repr(base_tail_A)} last_word={repr(base_last_word_A)}")
        elif stage == 1:
            base_tail_B, _ = extract_rime_ltr(line_ltr, stress_predict_fn=stress_predict_fn, stress_dict=stress_dict)
            base_last_word_B = extract_last_word_ltr(line_ltr)
            if debug:
                print(f"[ABAB] Line {line_no+1} base B tail={repr(base_tail_B)} last_word={repr(base_last_word_B)}")
